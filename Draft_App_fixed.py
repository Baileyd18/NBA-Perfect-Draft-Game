...
def mobile_roster_full():
    st.markdown(f'''<div class="mobile-budget-grid"><div class="mobile-budget-card"><div class="mobile-budget-label">Budget</div><div class="mobile-budget-value">${BUDGET}</div></div><div class="mobile-budget-card"><div class="mobile-budget-label">Spent</div><div class="mobile-budget-value">${st.session_state.spent}</div></div><div class="mobile-budget-card"><div class="mobile-budget-label">Remaining</div><div class="mobile-budget-value">${st.session_state.budget}</div></div></div><div style="color:#f8fafc;font-weight:900;margin-bottom:10px;">ROSTER ({len(st.session_state.roster)}/{ROSTER_SIZE})</div>''', unsafe_allow_html=True)
    roster_by_slot = {p["DraftPosition"]: p for p in st.session_state.roster}
    for slot in POSITIONS:
        p = roster_by_slot.get(slot)
        if p:
            st.markdown(f'''<div class="slot-card"><div class="slot-label">{slot}</div><div><div class="slot-player">{p["Player"]}</div><div class="slot-meta">{p["TeamAbbr"]} · {p["Decade"]} · Cost {int(p["Cost"])}</div></div></div>''', unsafe_allow_html=True)
        else:
            st.markdown(f'''<div class="slot-card"><div class="slot-label">{slot}</div><div class="slot-empty">Empty</div></div>''', unsafe_allow_html=True)
...
def render_mobile_app():
    ...
    with st.expander("Roster / Budget", expanded=False):
        mobile_roster_full()
    mobile_slot_dock()
    st.markdown('</div>', unsafe_allow_html=True)
